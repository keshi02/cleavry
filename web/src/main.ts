...
// ============================================================================
// Edge feathering — soften aliased alpha boundaries (e.g. hair tips left
// jagged by an AI cutout).
//
// Operates in *premultiplied alpha* so that feathered pixels inherit color
// from neighbouring opaque pixels rather than from RGB(0,0,0). Without this,
// AI-cutout PNGs (where transparent pixels have RGB=0) produce a black halo
// when feathered. The blur runs on (R*A, G*A, B*A, A); we unpremultiply at
// the end, dividing by the new alpha to recover the visible color.
//
// Pure function: takes RGBA bytes, returns a fresh RGBA buffer. Caller
// chooses when to apply (currently: at save / export time only).
// ============================================================================
function feather(src, W, H, iterations = 1) {
  const N = W * H;
...
// Convenience wrapper: returns workData with feather applied if the toggle
// is on, otherwise returns workData unchanged. Used by save / export paths.
